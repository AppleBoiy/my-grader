...
def print_test_results(num_passed: int, num_failed: int, failed_cases: List[Tuple], write_on_file: bool = False):
    """
    Print the test results as a table using the tabulate package and return the list of failed cases.
    ...
    if write_on_file:
        ...
        failed_cases_list = [{"Input": case[0], "Expected": case[1], "Got": case[2]} for case in failed_cases[:num_to_write]]
    ...
    if num_failed > 0:
        ...
        print(f"Total test cases: {num_passed + num_failed}")
